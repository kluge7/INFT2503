...
public:
  ...
  Cube() : shape({0.1, 0.1, 0.1}), body(1.0, &motion_state, &shape) {
    auto mass = (1.0 / body.getInvMass()) * 0.5;
...
public:
  ...
  // Add class parameters to store the drop position
  float horizontal_drop_position = 1.0f;
  float vertical_drop_position = 3.0f;
  ...
  World() : dispatcher(&collision_configuration),
    ...
    // Set initial positions for ground, spheres, and cubes (same as before)
  ...
  void reset() {
    ...
    horizontal_drop_position = 1.0f;
    vertical_drop_position = 3.0f;
  }

  void drop_ball() {
    btTransform transform;
    transform.setIdentity();
    transform.setOrigin(btVector3(horizontal_drop_position, vertical_drop_position, 0.0));
    falling_sphere.body.setCenterOfMassTransform(transform);
  }
...
public:
  ...
  void start() {
    ...
    while (running) {
      ...
      if (ImGui::SliderFloat("Horizontal ball position", &world.horizontal_drop_position, -2.0f, 4.0f)) {
      }

      if (ImGui::VSliderFloat("Vertical ball position", {20, 100}, &world.vertical_drop_position, 0.5f, 3.0f)) {
      }
